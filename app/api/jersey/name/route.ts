...
export async function POST(request: Request) {
  try {
    ...
    const response = await axios.get<ApiResponse>(`${API_BASE_URL}/jersey_name/${data.jersey_color_id || 1}`);

    if (response.data.result) {
      // Assuming `data` is an array of character names (strings)
      const matchedPlayers = data.data.map((charName: string) =>
        response.data.data.find((player: Player) => player.char === charName) || defaultPlayer
      );
